...
import { Star, Play, Layers2, ArrowLeft, Calendar, Users, TrendingUp, Heart, Bookmark, Share2, ChevronDown, ChevronUp, Tv, Award, Sparkles, Film, MessageCircle, ThumbsUp, Music, Zap, Globe, Flame, AlertCircle, Crown, MessageSquare, User, ChevronRight } from 'lucide-react';
...
export default function AnimeUI() {
  ...
  useEffect(() => {
    const fetchSimilarItems = async () => {
      if (!animeData?.similar_to || !Array.isArray(animeData.similar_to) || animeData.similar_to.length === 0) return;

      try {
        const similarResults = await Promise.all(
          animeData.similar_to.map(async (uid) => {
