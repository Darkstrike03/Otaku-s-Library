...
export default function MangaUI() {
  ...
  const navigateToAnime = () => {
    if (mangaData?.ani_uid) {
      router.push(`/details/${mangaData.ani_uid}`);
    }
  };
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-black' : 'bg-white'} overflow-hidden`}>
      ...
      {/* Hero Section */}
      <div className="relative h-[100vh] sm:h-[80vh] overflow-hidden group">
        <img
            ...
            <div className="lg:col-span-3 flex z-25 justify-center lg:justify-start - mt-8 lg:mb-0">
              <div className="relative group">
                ...
                <img
                  src={mangaData.poster}
                  alt={mangaData.title}
                  className="relative w-48 sm:w-56 md:w-64 lg:w-72 xl:w-80 h-64 sm:h-100 md:h-100 lg:h-96 xl:h-[28rem] rounded-2xl object-cover shadow-2xl"
                />
              </div>
            </div>
