...
import {
  Grid, List, Search, SlidersHorizontal, ChevronDown, Star, Clock, BookOpen, Tv, Book, Sparkles, TrendingUp, Filter, X, Heart, MessageCircle, Plus
} from 'lucide-react';
...
<button
  onClick={(e) => {
    e.stopPropagation();
    navigate(`/details/${item.id}`); // or `/anime/${item.id}` depending on your route structure
  }}
